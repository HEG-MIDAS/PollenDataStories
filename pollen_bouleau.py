...
for i in range(STARTING_YEAR, 2024):
    if i < 2023:
        mask = (df['time'] > datetime.datetime.strptime(str(i)+"-09-30", "%Y-%m-%d")) & (df['time'] <= datetime.datetime.strptime(str(i+1)+"-09-30", "%Y-%m-%d"))
        df_temp = df.loc[mask]
        pollen_avg.append([i+1, df_temp[df_temp['pollen_bouleau'] > 0].mean().values[0]])
        pollen_max.append([i+1, df_temp['pollen_bouleau'].max()])
...
print(df_pollen_max['pollen_bouleau_max'].mean())
